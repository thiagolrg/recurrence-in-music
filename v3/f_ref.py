...
def locR(linha,mapa,ppq):
    ref = locdur(linha,mapa[0])
    locR = (linha[1] - ref[0])/uc(ref,ppq)
    return locR

def locT(linha,mapa,ppq):
    ref = locdur(linha, mapa[0])
    locT = ((locR(linha,mapa,ppq)%1)*ref[1][0])+1
    return locT

def locC(linha,mapa,ppq):
    ref = locdur(linha,mapa[0])
    locC = int(locR(linha,mapa,ppq) + ref[2][0])
    return locC

def durR(linha,mapa,ppq):
    ref = locdur(linha,mapa[0])
    durR = locR(linha,mapa,ppq)*ref[1][0]
    return durR

def durI(linha,mapa,ppq):
    ref = locdur(linha,mapa[0])
    durI = durR(linha,mapa,ppq) + ref[3]
    return durI
